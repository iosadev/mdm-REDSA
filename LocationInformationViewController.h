//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "UIViewController.h"

@class UIBarButtonItem, UITextView;

@interface LocationInformationViewController : UIViewController
{
    UIBarButtonItem *_sidebarButton;
    UITextView *_informationTV;
}

@property(nonatomic) __weak UITextView *informationTV; // @synthesize informationTV=_informationTV;
@property(nonatomic) __weak UIBarButtonItem *sidebarButton; // @synthesize sidebarButton=_sidebarButton;
- (void).cxx_destruct;
- (void)didReceiveMemoryWarning;
- (long long)transferStringToDataSec:(id)arg1;
- (void)viewWillAppear:(_Bool)arg1;
- (void)viewDidLoad;

@end

